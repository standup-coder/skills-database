...
// Strategy: Look for pattern: knowledgePoints: [ followed by content and then ]\n          (10 spaces)
// This is the structure that ends a knowledgePoints array

// We'll look for "knowledgePoints: [" and then find the FIRST occurrence of "]\n          "
// (closing bracket with specific indentation) after it
...
while (true) {
  ...
  // Find category by looking backwards from kpIndex
  const segment = js.substring(Math.max(0, kpIndex - 400), kpIndex);
  const catMatch = segment.match(/category:\s*'([^']+)'/);

  if (catMatch) {
    const category = catMatch[1];

    // Find the closing - look for ] followed by 10 spaces
    const searchStart = kpIndex + 'knowledgePoints: ['.length;
    const closingPattern = ']\n          '; // ] with exactly 10 spaces after

    const closingIndex = js.indexOf(closingPattern, searchStart);

    if (closingIndex !== -1) {
      // Check if resources already follows
      const afterClosing = js.substring(closingIndex + closingPattern.length, closingIndex + closingPattern.length + 50);

      if (!/resources:/.test(afterClosing)) {
        // Insert resources here
        const resources = resourceTemplates[category] || resourceTemplates.default;
        ...
        js = js.substring(0, closingIndex) + insertStr + js.substring(closingIndex + closingPattern.length);
        inserted++;
      }
    }
  }
...
console.log(`Processed ${processed} knowledgePoints, inserted ${inserted} resources`);
