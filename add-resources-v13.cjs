...
// KEY INSIGHT:
// - With resources: knowledgePoints closing is "],\n          resources:"
// - Without resources: knowledgePoints closing is "]\n        },"
// The difference: comma after ] when resources exists

// Strategy: Find knowledgePoints: [ and track forward to find the closing
// If we find "]\n        }," (no comma before the closing), it's missing resources
...
while (pos < js.length) {
  ...
  if (catMatch) {
    // Scan forward from kpIdx to find the closing bracket of this kp array
    // We look for: first ] that is NOT followed by anything that could be inside the array
    // Actually, we need to track bracket depth

    let searchPos = kpIdx + 'knowledgePoints: ['.length;
    let depth = 1; // We've entered the array
    let kpEnd = -1;
    let lastValidKpEnd = -1;

    // Find the closing bracket
    for (let i = searchPos; i < js.length && i < searchPos + 10000; i++) {
      if (js[i] === '[') depth++;
      else if (js[i] === ']') {
        depth--;
        if (depth === 0) {
    ...
    if (kpEnd !== -1) {
      ...
      const afterClose = js.substring(kpEnd + 1, kpEnd + 50);

      // If the kp is followed by "],\n" (comma after bracket), then resources follow
      // If the kp is followed by "]\n" (no comma), then it goes straight to "},"
      // We need to check the pattern after close

      // The pattern WITHOUT resources is: ]\n        },
      // So we look for: ]\n        },
      const noResourcesPattern = ']\n        },';
      const hasResourcesPattern = '],\n          resources:';

      // Check if this kp closing is followed by resources or by end of object
      const contextAroundClose = js.substring(kpEnd, Math.min(kpEnd + 100, js.length));

      if (contextAroundClose.includes(noResourcesPattern)) {
        // This kp has no resources after it
        fixes.push({
          kpEnd: kpEnd,
          category: catMatch[1]
        });
