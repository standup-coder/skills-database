...
// NEW APPROACH:
// Look for the EXACT closing pattern of knowledgePoints:
// The closing of knowledgePoints array is: "]\n        " followed by either:
// - "}, " (end of skill object, no resources)
// - "],\n          resources: [" (has resources)
//
// So we find "knowledgePoints: [" and then look for the specific
...
while (pos < js.length) {
  ...
  // Get category by looking backwards
  const segment = js.substring(Math.max(0, kpIdx - 300), kpIdx);
  ...
  if (catMatch) {
    ...
    const closingIdx = js.indexOf(closingPattern, searchStart);

    if (closingIdx !== -1 && closingIdx < kpIdx + 5000) {
      // Check if resources follows
      const afterClosing = js.substring(closingIdx + closingPattern.length, closingIdx + closingPattern.length + 50);

      if (!/resources:/.test(afterClosing)) {
        fixes.push({
          kpEnd: closingIdx + closingPattern.length - 1, // position of ]
          category: catMatch[1]
        });
      }
    }
