...
// Strategy: find each skill block, find the pattern:
// knowledgePoints: [...],\n          ] (closing array)
// and we need to insert resources before the closing ]

// Actually the existing pattern is:
// knowledgePoints: [
//   ... items ...
// ]\n          resources: [...]

// So after knowledgePoints array ends with ], we have resources.
// I need to add resources after knowledgePoints: [ ... ] for skills that don't have it.

// Find skills without resources - look for knowledgePoints followed by atomicSkills or similar
// (not resources) within a reasonable distance

let processed = 0;
let lastEnd = 0;
const changes = [];

// Look for the pattern: knowledgePoints: [ ... ] where after ], there's no resources
// The key is the closing pattern "],\n          " followed by something that's not resources

// Find all knowledgePoints arrays
const kpRegex = /knowledgePoints:\s*\[([\s\S]*?)\]\s*(,\s*)/g;
let match;

while ((match = kpRegex.exec(js)) !== null) {
  const kpStart = match.index;
  const kpContent = match[0]; // includes "knowledgePoints: [" and closing "]\s*,"
  const kpEnd = kpStart + kpContent.length;
  const afterKp = js.substring(kpEnd, kpEnd + 100);

  // Check if resources follows
  if (!/resources:\s*\[/.test(afterKp)) {
    // Find the id - go backwards
    const beforeKp = js.substring(Math.max(0, kpStart - 600), kpStart);
    const idMatch = beforeKp.match(/id:\s*['"]([^'"]+)['"]/);
    const catMatch = beforeKp.match(/category:\s*['"]([^'"]+)['"]/);
    ...
    if (idMatch && catMatch) {
      changes.push({
        ...
        category: catMatch[1],
        kpEnd: kpEnd,
        afterKp: afterKp.substring(0, 100)
      });
    }
  }
}

console.log(`Found ${changes.length} skills needing resources`);

// Sort by position descending to apply changes from end to start
changes.sort((a, b) => b.kpEnd - a.kpEnd);

for (const change of changes) {
  const resources = resourceTemplates[change.category] || resourceTemplates.default;

  const resourcesStr = resources.map(r =>
    `            { title: '${r.title.replace(/'/g, "\\'")}', url: '${r.url}', type: 'doc', source: '${r.title.split(' ')[0]}' }`
  ).join(',\n');

  // Insert resources block after the ], that ends knowledgePoints
  // The pattern after kpEnd is: ],\n          (then atomicSkills or similar)
  // We need to change it to: ],\n          resources: [...],\n          (rest)

  const insertPoint = change.kpEnd;
  const afterInsert = js.substring(insertPoint);

  const newBlock = `],\n          resources: [\n${resourcesStr}\n          ]`;

  js = js.substring(0, insertPoint) + newBlock + afterInsert;

  processed++;
  if (processed % 20 === 0) {
    console.log(`Processed ${processed}/${changes.length}`);
  }
}
