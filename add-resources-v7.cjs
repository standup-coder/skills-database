...
// Find skills that have knowledgePoints but no resources
// Strategy: Find each skill object - look for id: pattern and track the object boundaries

// We'll scan for the specific pattern: id: 'xxx', ... category: 'xxx'
// Then look for knowledgePoints: [ in the next 200 chars
// If found, find the closing ] and check if resources follows
...
while (pos < js.length) {
  // Find "id: '" pattern
  const idMatch = js.indexOf("id: '", pos);
  if (idMatch === -1) break;

  // Get a segment after id to find category and knowledgePoints
  const segment = js.substring(idMatch, idMatch + 600);
  ...
  if (catMatch) {
    ...
    // Check if knowledgePoints exists in this segment
    const kpIdx = segment.indexOf('knowledgePoints: [');
    if (kpIdx !== -1) {
      // Find the full knowledgePoints array closing
      // Start counting from after the opening [
      let arrStart = idMatch + kpIdx + 'knowledgePoints: ['.length;
      let depth = 1;
      let arrEnd = -1;

      for (let i = arrStart; i < js.length && i < idMatch + 4000; i++) {
        if (js[i] === '[') depth++;
        else if (js[i] === ']') {
          depth--;
          if (depth === 0) {
      ...
      if (arrEnd !== -1) {
        ...
        const afterKp = js.substring(arrEnd + 1, arrEnd + 100);
