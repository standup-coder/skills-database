...
// Simple approach: find each occurrence of "knowledgePoints: ["
// and find its closing bracket, then check what follows

const kpPattern = 'knowledgePoints: [';
let pos = 0;
let processed = 0;
let inserted = 0;

while (true) {
  const kpIndex = js.indexOf(kpPattern, pos);
  if (kpIndex === -1) break;

  // Find the id and category by looking backwards
  const segment = js.substring(Math.max(0, kpIndex - 500), kpIndex);
  ...
  if (idMatch && catMatch) {
    ...
    // Find the closing bracket for this knowledgePoints array
    let depth = 1; // starting after the opening [
    let end = kpIndex + kpPattern.length;
    ...
    // Check if resources follows after the closing ]
    ...
    if (!/resources:\s*\[/.test(afterKp)) {
      ...
      inserted++;
      // Adjust pos since we inserted characters
      pos = end + insertStr.length;
    } else {
      pos = end + 1;
    }
  } else {
    pos = kpIndex + kpPattern.length;
  }
...
console.log(`Total: processed ${processed}, inserted ${inserted}`);
