...
// Find each skill object that has knowledgePoints but no resources
let processed = 0;
let offsetAdjustment = 0;

const skillRegex = /\{[\s\S]*?id:\s*['"]([^'"]+)['"][\s\S]*?name:\s*['"]([^'"]+)['"][\s\S]*?category:\s*['"]([^'"]+)['"][\s\S]*?knowledgePoints:\s*\[([\s\S]*?)\]([\s\S]*?)(?=\n\s*\{|\n\s*\/)/g;

let match;
const matches = [];

while ((match = skillRegex.exec(js)) !== null) {
  const [full, id, name, category, kpContent, afterKp] = match;
  const hasResources = /resources:\s*\[/.test(afterKp);
  ...
  if (!hasResources) {
    matches.push({
      ...
      kpEndOffset: full.indexOf(']', full.indexOf('knowledgePoints'))
    });
  }
}

console.log(`Found ${matches.length} skills without resources`);

// Sort by start position descending so we can replace from end to beginning
matches.sort((a, b) => b.start - a.start);

for (const skill of matches) {
  const resources = resourceTemplates[skill.category] || resourceTemplates.devops;
  ...
  // Find where the knowledgePoints array ends
  const skillSegment = js.substring(skill.start, skill.end);
  const kpArrayStart = skillSegment.indexOf('knowledgePoints: [');
  const kpArrayEnd = skillSegment.indexOf(']', kpArrayStart) + 1;

  // What's after the knowledgePoints array
  const afterKpArray = skillSegment.substring(kpArrayEnd);

  // Build the replacement - insert resources array after knowledgePoints ]
  const newSegment = skillSegment.substring(0, kpArrayEnd) + ',\n          resources: [\n' + resourcesStr + '\n          ]' + afterKpArray;

  // Apply the change
  js = js.substring(0, skill.start) + newSegment + js.substring(skill.end);
