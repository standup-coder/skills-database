...
// Find each skill - look for id: 'xxx' followed by category: 'yyy'
// and then knowledgePoints: [ which is not followed by resources

const skillRegex = /(\\{[^}]*?id:\s*'([^']+)'[^}]*?category:\s*'([^']+)'[^}]*?)knowledgePoints:\s*\[([^\]]+)\]([^}]*?)(\},|\s*\})/g;

let match;
const skillsToFix = [];

while ((match = skillRegex.exec(js)) !== null) {
  const [full, prefix, id, category, kpContent, rest, suffix] = match;
  const kpEndIndex = match.index + full.length - suffix.length;

  // Check if resources is in rest
  if (!/resources:\s*\[/.test(rest)) {
    skillsToFix.push({
      id,
      category,
      start: match.index,
      end: match.index + full.length,
      kpEndInFull: full.indexOf(']', full.indexOf('knowledgePoints: [')) + full.indexOf('knowledgePoints: [') + 'knowledgePoints: ['.length
    });
  }
}

console.log(`Found ${skillsToFix.length} skills to fix`);

// Sort by start descending
skillsToFix.sort((a, b) => b.start - a.start);
...
for (const skill of skillsToFix) {
  ...
  // Find the knowledgePoints closing in the original js
  const kpStart = js.indexOf('knowledgePoints: [', skill.start);
  if (kpStart === -1 || kpStart > skill.start + 500) continue;

  // Find the closing ] by counting
  let depth = 1; // we've seen the opening [
  let end = kpStart + 'knowledgePoints: ['.length;
  ...
  // Check if resources already exists after this
  const afterEnd = js.substring(end + 1, end + 100);
  if (/resources:\s*\[/.test(afterEnd)) continue;

  // Insert resources after the ]
  const insertStr = `],\n          resources: [\n${resourcesStr}\n          ]`;
  js = js.substring(0, end) + insertStr + js.substring(end + 1);
