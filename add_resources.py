#!/usr/bin/env python3
"""
Script to add resources array to skills that have knowledgePoints but no resources.
"""
import re

# Define resources for each skill category/topic
RESOURCES_MAP = {
    # SRE Core
    ...
    'k8s-troubleshooting': [
        ...
        { 'title': 'CoreDNS 故障排查', 'url': 'https://kubernetes.io/docs/tasks/network/validate-n setup/, 'type': 'doc', 'source': 'CNCF' },
    ...
    'service-health-check': [
        ...
        { 'title': 'gRPC 健康检查协议', 'url': 'github.com/grpc/grpc/blob/master/doc/health-checking.md', 'type': 'doc', 'source': 'gRPC' },
    ...
    # DevOps Skills (that don't have resources)
    ...
    # Backend Skills (that don't have resources)
    'api-design': [
    ...
    # Frontend Skills (that don't have resources)
    'component-design': [
    ...
    # QA Skills (that don't have resources)
    'write-unit-tests': [
    ...
    # PM Skills (that don't have resources)
    'write-prd': [
    ...
    # Cloud Security Skills (that don't have resources)
    ...
    'secrets-management': [
        ...
        { 'title': '密钥轮转最佳实践', 'https://developer.hashicorp.com/vault/docs/secretsrotation', 'type': 'tutorial', 'source': 'HashiCorp' }
    ],
    'config-validator': [
    ...
    # Auth and API Dev (missing resources)
    'api-development': [
...
def format_resources_array(resources):
    """Format a resources array for insertion."""
    formatted = "          resources: [\n"
    for r in resources:
        formatted += f"            {{ title: '{r['title']}', url: '{r['url']}', type: '{r['type']}', source: '{r['source']}' }},\n"
    formatted += "          ]"
    return formatted
...
def process_file(filepath):
    ...
    # Find all skills with knowledgePoints
    # Pattern: { id: 'xxx', ..., knowledgePoints: [...], <no resources> }
    # We need to find knowledgePoints sections that are NOT followed by resources:

    # Split by skills - each skill starts with { id: '
    skill_pattern = r"(\{{ id: '([^']+)',[^}]*knowledgePoints: \[([\s\S]*?)\]\n        \})"

    def replace_skill(match):
        full_match = match.group(0)
        skill_id = match.group(2)
        knowledge_content = match.group(3)

        # Check if this skill already has resources
        # Look at what comes after the knowledgePoints ]
        after_kp = match.group(0)[match.group(0).find(']') + len(']'):]

        if 'resources:' in after_kp[:100]:
            # Already has resources, don't modify
            return full_match

        # This skill needs resources
        if skill_id in RESOURCES_MAP:
            resources = RESOURCES_MAP[skill_id]
            resources_str = format_resources_array(resources)

            # Find the closing of knowledgePoints and replace
            # The pattern ends with: knowledgePoints: [...]]
            # We need to add resources before the final }

            # Find the position where knowledgePoints ends and we need to add resources
            idx = full_match.rfind(']')
            if idx != -1:
                # Find what comes after the ]
                remaining = full_match[idx+1:].strip()
                if remaining.startswith('}'):
                    # This is the end of the skill object
                    # Insert resources before the closing }
                    new_skill = full_match[:idx+1] + '\n' + resources_str + '\n        ' + remaining
                    return new_skill

        return full_match

    # This regex is complex, let me use a more targeted approach
    # Find knowledgePoints blocks and check if they have resources after
    lines = content.split('\n')
    result_lines = []
    i = 0
    ...
    while i < len(lines):
        ...
        # Detect start of a skill
        id_match = re.match(r"\s*\{\s*id:\s*'([^']+)'", line)
        if id_match:
            current_skill_id = id_match.group(1)

        # Check for knowledgePoints
        if 'knowledgePoints:' in line and current_skill_id:
            # Found knowledgePoints, capture the block
            kp_start = i
            bracket_count = 0
            kp_end = -1

            # Find where knowledgePoints array ends
            for j in range(i, len(lines)):
                if 'knowledgePoints: [' in lines[j]:
                    # Start of array
                    bracket_count = 1
            ...
            if kp_end > kp_start:
                # Check if resources follows
                after_kp_lines = lines[kp_end+1:kp_end+3]
                has_resources = any('resources:' in l for l in after_kp_lines)

                if not has_resources and current_skill_id in RESOURCES_MAP:
                    # Insert resources
                    resources = RESOURCES_MAP[current_skill_id]
                    # Format the resources
                    resources_block = ["          resources: ["]
                    for r in resources:
                        resources_block.append(f"            {{ title: '{r['title']}', url: '{r['url']}', type: '{r['type']}', source: '{r['source']}' }},")
                    resources_block.append("          ]")

                    # Insert resources block
                    for rl in resources_block:
                        result_lines.append(rl)
                    result_lines.append(lines[j] for j in range(kp_start, kp_end+1))
                    i = kp_end + 1
                    continue
...
# Let me take a different approach - find each skill and check/insert resources
def process_file_v2(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Find all skill blocks using a more reliable approach
    # Each skill is delimited by { id: '...' at the start and }, at the end
    ...
    while i < len(lines):
        line = lines[i]
        # Start of a skill object
        if re.match(r"\s*\{\s*id:\s*'", line):
            # Collect this skill block
            skill_start = i
            brace_count = 0
            in_string = False
            escape_next = False
            ...
            if skill_end != -1:
                skill_lines = lines[skill_start:skill_end+1]
                skill_blocks.append({
                    'start': skill_start,
                    'end': skill_end,
                    'lines': skill_lines,
    ...
    print(f"Found {len(skill_blocks)} skill blocks")

    return content

if __name__ == '__main__':
    # This approach is complex, let me use string replacement
    print("Use the string matching approach")