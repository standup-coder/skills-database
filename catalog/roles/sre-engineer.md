---
id: sre-engineer
type: role
title: SRE Engineer
nameZh: SRE 工程师
domain: devops
level: senior
tags: sre, reliability, monitoring, observability, incident-response
catalogSource: internal
catalogFile: roles/sre-engineer.json
catalogAddedAt: 2026-07-26
experience: 4+ years
responsibilities: 定义和追踪 SLI/SLO/Error Budget | 搭建和维护监控告警体系 | 主导故障排查与事后复盘 | 推动自动化以减少重复劳动
mainSkills: terraform-plan-review, docker-container-management, k8s-deployment-review, ci-pipeline-setup, monitoring-stack-setup, incident-diagnosis, security-audit, log-analysis
atomicSkills: read-file, write-file, run-shell-command, http-health-check, parse-json-log, docker-exec, validate-k8s-manifest, api-call, sre-engineer, prometheus-monitoring, grafana-dashboard, logging-observability, incident-runbook, cost-optimization, platform-healthcheck, k8s-security, k8s-troubleshooting, security-scan, log-parser, service-health-check, system-watchdog
---

# SRE 工程师

> 保障生产系统的高可用性、可观测性与快速恢复能力

## 职责

- 定义和追踪 SLI/SLO/Error Budget
- 搭建和维护监控告警体系
- 主导故障排查与事后复盘
- 推动自动化以减少重复劳动

## 核心能力(主 Skills)

- [terraform-plan-review](../devops/terraform-plan-review.md) — 一句话能力
- [docker-container-management](../devops/docker-container-management.md) — 一句话能力
- [k8s-deployment-review](../devops/k8s-deployment-review.md) — 一句话能力
- [ci-pipeline-setup](../devops/ci-pipeline-setup.md) — 一句话能力
- [monitoring-stack-setup](../devops/monitoring-stack-setup.md) — 一句话能力
- [incident-diagnosis](../devops/incident-diagnosis.md) — 一句话能力
- [security-audit](../devops/security-audit.md) — 一句话能力
- [log-analysis](../devops/log-analysis.md) — 一句话能力

## 基础操作(原子 Skills)

- [read-file](../productivity/read-file.md) — 一句话能力
- [write-file](../productivity/write-file.md) — 一句话能力
- [run-shell-command](../backend/run-shell-command.md) — 一句话能力
- [http-health-check](../security/http-health-check.md) — 一句话能力
- [parse-json-log](../data/parse-json-log.md) — 一句话能力
- [docker-exec](../devops/docker-exec.md) — 一句话能力
- [validate-k8s-manifest](../testing/validate-k8s-manifest.md) — 一句话能力
- [api-call](../security/api-call.md) — 一句话能力
- [sre-engineer](./sre-engineer.md) — 一句话能力
- [prometheus-monitoring](../devops/prometheus-monitoring.md) — 一句话能力
- [grafana-dashboard](../devops/grafana-dashboard.md) — 一句话能力
- [logging-observability](../devops/logging-observability.md) — 一句话能力
- [incident-runbook](../devops/incident-runbook.md) — 一句话能力
- [cost-optimization](../devops/cost-optimization.md) — 一句话能力
- [platform-healthcheck](../devops/platform-healthcheck.md) — 一句话能力
- [k8s-security](../security/k8s-security.md) — 一句话能力
- [k8s-troubleshooting](../devops/k8s-troubleshooting.md) — 一句话能力
- [security-scan](../security/security-scan.md) — 一句话能力
- [log-parser](../devops/log-parser.md) — 一句话能力
- [service-health-check](../devops/service-health-check.md) — 一句话能力
- [system-watchdog](../devops/system-watchdog.md) — 一句话能力

## 经验门槛

| 维度 | 要求 |
|------|------|
| 经验 | 4+ years |
| 学历 | - |
| 核心技术 | Kubernetes, Prometheus, Grafana, Terraform, Incident Management |

## 学习路径

### Junior → Mid

- 掌握所有 atomicSkills 列出的基础操作
- 能独立完成 mainSkills 中 1-2 个简单工作流
- 在指导下进行 code review

### Mid → Senior

- 能设计新的 composite skill 流程
- 主导中小型项目
- 指导 Junior

### Senior → Lead

- 能制定岗位标准与技术战略
- 跨团队协作与架构决策
- 培养下一个层级的接班人

## 相关角色

_此节由后续 skill-relationship 工具生成_
