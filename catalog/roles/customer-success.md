---
id: customer-success
type: role
title: Customer Success Manager
nameZh: 客户成功经理
domain: product
level: mid
tags: customer-success, support, retention, onboarding, cs
catalogSource: internal
catalogFile: roles/customer-success.json
catalogAddedAt: 2026-07-26
experience: 3+ years
responsibilities: 引导新客户完成 onboarding | 监控客户健康度和使用情况 | 识别流失风险并制定挽留策略 | 收集客户反馈推动产品改进 | 管理客户关系和定期业务回顾
mainSkills: product-research
atomicSkills: read-file, write-file, api-call, onboarding, health-monitoring, retention, feedback-loop, stakeholder-management
---

# 客户成功经理

> 管理客户生命周期，推动用户 onboarding、留存续费、价值实现

## 职责

- 引导新客户完成 onboarding
- 监控客户健康度和使用情况
- 识别流失风险并制定挽留策略
- 收集客户反馈推动产品改进
- 管理客户关系和定期业务回顾

## 核心能力(主 Skills)

- [product-research](../product/product-research.md) — 一句话能力

## 基础操作(原子 Skills)

- [read-file](../productivity/read-file.md) — 一句话能力
- [write-file](../productivity/write-file.md) — 一句话能力
- [api-call](../security/api-call.md) — 一句话能力
- [onboarding](../product/onboarding.md) — 一句话能力
- [health-monitoring](../devops/health-monitoring.md) — 一句话能力
- [retention](../marketing/retention.md) — 一句话能力
- [feedback-loop](../product/feedback-loop.md) — 一句话能力
- [stakeholder-management](../product/stakeholder-management.md) — 一句话能力

## 经验门槛

| 维度 | 要求 |
|------|------|
| 经验 | 3+ years |
| 学历 | - |
| 核心技术 | Customer Onboarding, Retention, Communication, Analytics, Empathy |

## 学习路径

### Junior → Mid

- 掌握所有 atomicSkills 列出的基础操作
- 能独立完成 mainSkills 中 1-2 个简单工作流
- 在指导下进行 code review

### Mid → Senior

- 能设计新的 composite skill 流程
- 主导中小型项目
- 指导 Junior

### Senior → Lead

- 能制定岗位标准与技术战略
- 跨团队协作与架构决策
- 培养下一个层级的接班人

## 相关角色

_此节由后续 skill-relationship 工具生成_
