...
// knowledgePoints: [\n...items...\n          ]\n          (something that's not resources)
// or knowledgePoints: [\n...items...\n          resources: [
// For skills without resources, we see: knowledgePoints: [ ... ]\n        },
// For skills with resources, we see: knowledgePoints: [ ... ],\n          resources: [

// Find all skill blocks and check if they have resources after knowledgePoints

// Use a state machine to find each skill's knowledgePoints closing
let pos = 0;
let skillsWithoutResources = [];

while (pos < js.length) {
  // Look for "knowledgePoints: [" pattern
  const kpIndex = js.indexOf('knowledgePoints: [', pos);
  if (kpIndex === -1) break;

  // Find the matching closing bracket
  let depth = 0;
  let end = kpIndex + 'knowledgePoints: ['.length;
  for (let i = kpIndex + 'knowledgePoints: ['.length; i < js.length; i++) {
    if (js[i] === '[') depth++;
    else if (js[i] === ']') {
      if (depth === 0) {
        end = i;
  ...
  // After the closing ], check what's next
  const afterKp = js.substring(end + 1, end + 100);

  // Find the skill's id by looking backwards
  const beforeKp = js.substring(Math.max(0, kpIndex - 600), kpIndex);
...
console.log(`Found ${skillsWithoutResources.length} skills needing resources`);

// Sort by position descending
skillsWithoutResources.sort((a, b) => b.kpEnd - a.kpEnd);

// Process each
let processed = 0;
for (const skill of skillsWithoutResources) {
  ...
  // Insert after kpEnd: "],\n          resources: [\n...]\n"
  const insertAt = skill.kpEnd;
  const insertStr = `],\n          resources: [\n${resourcesStr}\n          ]`;

  js = js.substring(0, insertAt) + insertStr + js.substring(insertAt + 1);

  processed++;
  if (processed % 20 === 0) {
    console.log(`Processed ${processed}/${skillsWithoutResources.length}`);
  }
}
