#!/usr/bin/env python3
"""Script to add resources to skills that have knowledgePoints but no resources."""

import re

# Resources for each skill ID
SKILL_RESOURCES = {
...
def add_resources_to_file(filepath):
    ...
    while i < len(lines):
        ...
        # Detect a skill block start by looking for { id: '
        if re.match(r"\s*\{\s*id:\s*'", line):
            ...
            # Parse through the skill to find its boundaries
            ...
            skill_text = '\n'.join(skill_lines)
            skill_end = i + len(skill_lines) - 1

            # Check if this skill has knowledgePoints
            if 'knowledgePoints:' in skill_text:
                # Extract skill ID
                id_match = re.search(r"id:\s*'([^']+)'", skill_text)
                skill_id = id_match.group(1) if id_match else None

                # Check if it already has resources
                has_resources = 'resources:' in skill_text

                # Check if we have resources for this skill
                if skill_id and skill_id in SKILL_RESOURCES and not has_resources:
                    # Find the closing of knowledgePoints array
                    kp_end_idx = None
                    for li, sl in enumerate(skill_lines):
                        if ']' in sl and 'knowledgePoints' not in sl:
                            # Check if this ] closes knowledgePoints
                            # Find the line containing knowledgePoints: [
                            kp_line = None
                            for kpl in skill_lines:
                                if 'knowledgePoints: [' in kpl:
                                    kp_line = kpl
                                    break
                            if kp_line:
                                kp_end_idx = li
                    ...
                    if kp_end_idx is not None:
                        # Find the line after knowledgePoints ] to check it's the skill end
                        # Insert resources before the skill closing
                        # The pattern is: knowledgePoints: [...] ]
                        # And should be followed by }, (closing the skill)

                        # Check if the line after kp_end_idx is just }
                        if kp_end_idx + 1 < len(skill_lines):
                            next_line = skill_lines[kp_end_idx + 1].strip()
                            if next_line == '},' or next_line == '}':
                                # Insert resources
                                resources_str = format_resources(SKILL_RESOURCES[skill_id])
                                # Insert the resources lines
                                for ri, rl in enumerate(resources_str.split('\n')):
                                    result.append(rl)
                                result.append('        },')
                                i = skill_end + 1
                                continue

            # If we didn't modify, add all skill lines
...
if __name__ == '__main__':
    ...
    new_content = add_resources_to_file(filepath)
