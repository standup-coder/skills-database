...
export class AgentRuntime extends EventEmitter {
     ...
     * 执行复合技能（加载工作流）
     */
    async executeCompositeSkill(skillId, inputs, agentId) {
        ...
        for (const step of skillDef.workflow.steps) {
            ...
            try {
                ...
                if (skillDef.errorHandling?.strategy === 'stop') {
                    ...
                    const fallbackStep = skillDef.workflow.steps.find((s) => s.id === skillDef.errorHandling.fallback.step);
                    if (fallbackStep) {
     ...
     * 执行原子技能
     */
    async executeAtomicSkill(skillId, inputs) {
     ...
     * 执行 LLM 步骤
     */
    async executeLlmStep(step, inputs, agentId) {
        // 获取 chat 工具
     ...
     * 执行转换步骤
     */
    executeTransformStep(step, inputs) {
        // 数据转换占位
        return {
            step: step.id,
            type: 'transform',
     ...
     * 检查安全约束
     */
    checkConstraints(inputs, constraints) {
        if (!constraints)
            return;
