...
// Find each skill object and check if it has knowledgePoints but no resources
// Strategy: find skill objects, extract category, find knowledgePoints array,
// find its closing bracket, check if resources follows after

const skillRegex = /\{[\s\S]*?id:\s*['"]([^'"]+)['"][\s\S]*?category:\s*['"]([^'"]+)['"][\s\S]*?knowledgePoints:\s*\[([\s\S]*?)\]([\s\S]*?)(?=\n\s*\{|\n\s*\/\/)/g;

let match;
const skillsToFix = [];

while ((match = skillRegex.exec(js)) !== null) {
  const [full, id, category, kpContent, afterKp] = match;
  const kpStart = match.index;
  const kpArrayStart = full.indexOf('knowledgePoints: [');
  const kpArrayContentStart = kpArrayStart + 'knowledgePoints: ['.length;
  const kpArrayEnd = kpArrayStart + full.indexOf(']', full.indexOf('knowledgePoints: ['));

  // Check if resources exists in afterKp
  if (!/resources:\s*\[/.test(afterKp)) {
    // Find where in js this skill is
    const skillStart = kpStart;
    const skillEnd = kpStart + full.length;
    skillsToFix.push({
      id,
      category,
      skillStart,
      skillEnd,
      kpArrayStart: kpStart + kpArrayStart,
      kpArrayEnd: kpStart + kpArrayEnd
    });
  }
}

console.log(`Found ${skillsToFix.length} skills needing resources`);

// Sort by position descending
skillsToFix.sort((a, b) => b.skillStart - a.skillStart);

// Process each skill
let processed = 0;
for (const skill of skillsToFix) {
  const resources = resourceTemplates[skill.category] || resourceTemplates.default;
  ...
  // Find the knowledgePoints array end in the original js
  // We need to insert resources after the ] that closes knowledgePoints
  // But BEFORE any subsequent property like atomicSkills or steps

  // Get the segment containing this skill
  const segment = js.substring(skill.kpArrayStart, skill.kpArrayEnd + 1);

  // Find the closing ] of knowledgePoints by counting brackets
  // The content between knowledgePoints: [ and ] should have balanced brackets in items
  // But the items are objects { ... } with their own brackets

  // Let's find the actual closing bracket by scanning forward from kpArrayStart
  let depth = 0;
  let kpEnd = -1;
  for (let i = skill.kpArrayStart; i < js.length; i++) {
    if (js[i] === '[') depth++;
    else if (js[i] === ']') {
      depth--;
      if (depth === 0) {
  ...
  if (kpEnd === -1) {
    console.log(`Could not find kp end for ${skill.id}`);
    continue;
  }

  // Check what's after kpEnd
  const afterKpEnd = js.substring(kpEnd + 1, kpEnd + 100);

  // If resources already exists, skip
  if (/resources:\s*\[/.test(afterKpEnd)) {
    continue;
  }

  // Insert resources block after kpEnd
  const resourcesBlock = `],\n          resources: [\n${resourcesStr}\n          ]`;

  js = js.substring(0, kpEnd) + resourcesBlock + js.substring(kpEnd + 1);

  processed++;
  if (processed % 20 === 0) {
    console.log(`Processed ${processed}/${skillsToFix.length}`);
  }
}
