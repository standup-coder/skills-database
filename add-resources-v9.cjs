...
// Strategy: Find each skill and the knowledgePoints closing
// For each knowledgePoints, find its closing ] and then check what property follows
// If resources doesn't follow within a reasonable distance, insert it

// Find each skill by looking for id: pattern and tracking forward
let pos = 0;
let fixes = [];

while (pos < js.length) {
  // Find "id: '" pattern
  const idIdx = js.indexOf("id: '", pos);
  if (idIdx === -1) break;

  // Get segment to find category and knowledgePoints
  const segment = js.substring(idIdx, idIdx + 800);
  const catMatch = segment.match(/category:\s*'([^']+)'/);
  const kpIdx = segment.indexOf('knowledgePoints: [');

  if (catMatch && kpIdx !== -1) {
    const category = catMatch[1];

    // Find the closing bracket of knowledgePoints
    // Start from after the opening [
    const arrStart = idIdx + kpIdx + 'knowledgePoints: ['.length;
    let depth = 1;
    let kpEnd = -1;

    for (let i = arrStart; i < js.length && i < arrStart + 10000; i++) {
      if (js[i] === '[') depth++;
      else if (js[i] === ']') {
        depth--;
        if (depth === 0) {
    ...
    if (kpEnd !== -1) {
      // Check what's after kpEnd - look for resources property or end of object
      const afterKp = js.substring(kpEnd + 1, kpEnd + 200);

      // If resources is NOT found in the next 200 chars, we need to add it
      if (!/resources:\s*\[/.test(afterKp)) {
        fixes.push({
          kpEnd: kpEnd,
          category: category,
          afterKpSample: afterKp.substring(0, 50)
        });
      }
    }
  }
