...
// Look for the specific pattern: knowledgePoints: [ ... ]\n        },
// where the closing ] is followed by 8 spaces and then }
// This indicates the skill has knowledgePoints but no resources

// The pattern to find is the sequence:
// knowledgePoints: [\n ... items ... \n        ]\n        },
// Without resources: the ]\n        }, appears directly
// With resources: the ]\n        ],\n          resources: [ appears

// Approach: find "knowledgePoints: [" and then scan forward until we find
// the EXACT pattern: "]\n        }" (closing with no resources) or "],\n          resources:"

// Actually, I need to find the ACTUAL closing bracket of knowledgePoints
// by tracking from knowledgePoints: [ forward

// The key insight: the knowledgePoints array ends with "]\n        " followed by either:
// - "resources: [" (if resources exists)
// - "}, " or "}\n        " (if no resources, end of skill object)

// Let's find knowledgePoints and scan for its closing
...
while (pos < js.length) {
  ...
  // Find category - look backwards
  const segmentBefore = js.substring(Math.max(0, kpIdx - 300), kpIdx);
  const catMatch = segmentBefore.match(/category:\s*'([^']+)'/);

  if (catMatch) {
    // Now find the closing bracket of this knowledgePoints
    // We start from after "knowledgePoints: ["
    let searchPos = kpIdx + 'knowledgePoints: ['.length;
    let depth = 1;

    // Find the closing ]
    let kpEnd = -1;
    for (let i = searchPos; i < js.length - 10; i++) {
      if (js[i] === '[') depth++;
      else if (js[i] === ']') {
        depth--;
        if (depth === 0) {
    ...
    if (kpEnd !== -1) {
      ...
      const afterKp = js.substring(kpEnd + 1, kpEnd + 50);

      // If resources doesn't follow within the next 50 chars, we need to add it
      if (!/resources:\s*\[/.test(afterKp)) {
        fixes.push({
          kpEnd: kpEnd,
          category: catMatch[1],
          afterSample: afterKp.substring(0, 30)
        });
      }
    }
  }
...
// Sort by kpEnd descending (insert from end to start)
