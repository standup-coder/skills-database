...
export class SkillHubAdapter {
  ...
  private loadCompositeSkills(): void {
    ...
    for (const file of files) {
      try {
        ...
        this.registerSkill(data, descriptor, filePath);
      } catch (error) {
  ...
  /**
   * 冲突保护：若同一 id 已作为 composite 存在，则跳过（保留 composite 为权威），
   * 避免原子技能静默覆盖复合技能。返回 true 表示已记录冲突。
   */
  private registerSkill(data: { id: string }, descriptor: SkillDescriptor, source: string): void {
    const existing = this.skillsCache.get(data.id);
    if (existing && existing.type !== descriptor.type) {
      console.warn(
        `[SkillHub] ID collision: "${data.id}" exists as both ${existing.type} and ${descriptor.type}. ` +
          `Keeping ${existing.type} (from ${existing.filePath}); ignoring ${descriptor.type} (${source}).`
      );
      return;
    }
  ...
  /**
   * 检测角色技能引用的类型错配：
   *   mainSkills    应指向 composite-skill，若指向 atomic-skill 则记为 mainRefersAtomic
   *   atomicSkills  应指向 atomic-skill，若指向 composite-skill 则记为 atomicRefersComposite
   * 返回三类：错配项 + 不存在的引用（missing）。
   */
  getSkillTypeMismatches(roleId: string): {
    mainRefersAtomic: string[];
    atomicRefersComposite: string[];
    missing: Array<{ id: string; expected: 'composite-skill' | 'atomic-skill' }>;
  } {
    if (!this.initialized) this.initialize();
    const role = this.rolesCache.get(roleId);
    if (!role) {
      return { mainRefersAtomic: [], atomicRefersComposite: [], missing: [] };
    }

    const mainRefersAtomic: string[] = [];
    const atomicRefersComposite: string[] = [];
    const missing: Array<{ id: string; expected: 'composite-skill' | 'atomic-skill' }> = [];

    for (const skillId of role.mainSkills) {
      const skill = this.skillsCache.get(skillId);
      if (!skill) missing.push({ id: skillId, expected: 'composite-skill' });
      else if (skill.type === 'atomic-skill') mainRefersAtomic.push(skillId);
    }

    for (const skillId of role.atomicSkills) {
      const skill = this.skillsCache.get(skillId);
      if (!skill) missing.push({ id: skillId, expected: 'atomic-skill' });
      else if (skill.type === 'composite-skill') atomicRefersComposite.push(skillId);
    }
